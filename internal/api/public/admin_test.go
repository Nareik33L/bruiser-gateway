...
func TestAdminStatusAndAuditExport(t *testing.T) {
	...
	resp.Body.Close()
	if resp.StatusCode != 200 || !strings.Contains(string(html), "Observed EAF") || !strings.Contains(string(html), "Queue depth") || !strings.Contains(string(html), "Not a waiting room") {
